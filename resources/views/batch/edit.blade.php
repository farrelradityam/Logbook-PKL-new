...
<x-slot:title>{{$title}}</x-slot:title>
    <form action="{{ route('batch.update', $batch->id) }}" method="POST" class="space-y-4">
        ...
        <div class="max-w-1g border border-slate-400 rounded-xl mx-auto shadow-lg font-inter p-5">
            ...
            <input type="text" id="year" name="year" value="{{ $batch->year }}" class="px-3 py-2 border shadow rounded w-full block text-sm focus:outline-none focus:ring-1 focus:ring-slate-500 focus:border-slate-500" required />
        </div>
